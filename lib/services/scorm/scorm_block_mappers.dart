...
class HtmlGenerator {
  ...
  String generateModulePage(
    ...
    final nav = '''
      <div class="nav-area">
        ${idx > 0 
            ? '<a href="module_${idx - 1}.html" class="btn-nav sec magnetic" onclick="return navigateModule(\'module_${idx - 1}.html\')">← Anterior</a>' 
            : '<span></span>'}
        
        ${idx < course.modules.length - 1 
            ? '<a href="module_${idx + 1}.html" class="btn-nav pri magnetic" onclick="return navigateModule(\'module_${idx + 1}.html\')">Siguiente →</a>' 
            : '<button class="btn-nav success magnetic" onclick="launchConfetti(80); alert(\'¡Módulo Finalizado! Puedes repasar o ir al panel.\')">Finalizar Módulo 🎉</button>'}
      </div>''';

    final body = '''
      <h1 class="page-title">$title</h1>
      <hr class="divider">
      ${renderBlocks(module.blocks, assetMap: assetMap)}
      $nav
    ''';

    return ScormUiArchitect.buildPage(title: title, bodyContent: body, progress: progress);
  }
  ...
  String generateStaticPage(
    ...
    ''';
    return ScormUiArchitect.buildPage(title: title, bodyContent: body, progress: 0);
  }

  String renderBlocks(List<InteractiveBlock> blocks, {Map<String, String>? assetMap}) {
    ...
    for (var block in blocks) {
      ...
      buf.writeln('<section class="block-wrap staggered" id="block-$safeId" data-block-label="$label" data-block-index="$blockIndex" data-block-kind="$kind"><div class="card-educativa">');
      ...
      if (htmlBlock.trim().isNotEmpty) {
        ...
        buf.writeln('<div class="block generic">[Error de Renderizado en bloque ${type.name}]</div>');
      } else {
  ...
  String _renderBlockHtml({
    ...
    if (type == BlockType.textPlain || type == BlockType.textRich || type == BlockType.essay) {
      ...
      return '<div class="block text-block">$safeText</div>';
      ...
      final url = _resolveAsset(_stringFrom(content, const ['url', 'imageUrl', 'imagePath', 'path', 'src']) ?? '', assetMap);
      final caption = _stringFrom(content, const ['caption', 'title', 'alt']) ?? '';
      final imageHtml = url.isNotEmpty ? '<img src="$url" alt="Imagen">' : '';
      final captionHtml = caption.isNotEmpty ? '<p class="caption">$caption</p>' : '';
      final hotspots = _listFrom(content, const ['hotspots', 'points', 'areas']);
      ...
      final url = _resolveAsset(_stringFrom(content, const ['url', 'videoUrl', 'videoPath', 'path', 'src']) ?? '', assetMap);
      ...
      final url = _resolveAsset(_stringFrom(content, const ['url', 'audioUrl', 'audioPath', 'path', 'src']) ?? '', assetMap);
      final title = _stringFrom(content, const ['title', 'label', 'name']) ?? 'Audio';
      ...
      final url = _resolveAsset(_stringFrom(content, const ['url', 'pdfUrl', 'pdfPath', 'path', 'src']) ?? '', assetMap);
      ...
      final url = _stringFrom(content, const ['url', 'link', 'href', 'src']) ?? '';
      final label = _stringFrom(content, const ['label', 'title', 'text']) ?? 'Abrir recurso';
      ...
      final code = _stringFrom(content, const ['code', 'embed', 'embedCode', 'html', 'iframe']) ?? '';
      ...
      final text = _stringFrom(content, const ['text', 'quote', 'content', 'description']) ?? '';
      final author = _stringFrom(content, const ['author', 'by', 'source']) ?? '';
      ...
      final buf = StringBuffer('<div class="block timeline-block"><h3>Línea de Tiempo</h3><div class="timeline-container">');
      for (final e in events) {
        ...
        buf.writeln('<div class="timeline-item"><div class="tl-date">$label</div><div class="tl-content"><h4>$title</h4><p>$desc</p></div></div>');
      }
      buf.writeln('</div></div>');
      return buf.toString();
      ...
      final buf = StringBuffer('<div class="block process-block"><h3>Proceso Paso a Paso</h3><div class="process-list">');
      int idx = 1;
      for (final s in steps) {
        final map = (s is Map) ? s.cast<String, dynamic>() : <String, dynamic>{};
        final title = (map['title'] ?? 'Paso $idx').toString();
        final desc = (map['desc'] ?? map['description'] ?? '').toString();
        buf.writeln('<div class="process-step"><div class="step-num">$idx</div><div class="step-text"><h4>$title</h4><p>$desc</p></div></div>');
        idx++;
      }
      buf.writeln('</div></div>');
      return buf.toString();
      ...
      final items = _listFrom(content, const ['items', 'sections', 'accordionItems']);
      ...
      for (final item in items) {
        ...
        final body = (map['content'] ?? map['body'] ?? map['description'] ?? '').toString();
        buf.writeln('<details><summary>$title</summary><div class="details-content">$body</div></details>');
      }
      buf.writeln('</div>');
      return buf.toString();
      ...
      final buf = StringBuffer('<div class="block tabs-block"><div class="tabs-header">');
      for (int i = 0; i < tabs.length; i++) {
        final map = (tabs[i] is Map) ? (tabs[i] as Map).cast<String, dynamic>() : <String, dynamic>{};
        final title = (map['title'] ?? map['label'] ?? 'Pestaña ${i + 1}').toString();
        buf.writeln('<button class="tab-btn" onclick="openTab(\'$safeId\', $i)">$title</button>');
      }
      buf.writeln('</div>');
      for (int i = 0; i < tabs.length; i++) {
        final map = (tabs[i] is Map) ? (tabs[i] as Map).cast<String, dynamic>() : <String, dynamic>{};
        final body = (map['content'] ?? map['body'] ?? map['description'] ?? '').toString();
        buf.writeln('<div id="tab-$safeId-$i" class="tab-content" style="display:${i == 0 ? "block" : "none"}">$body</div>');
      }
      buf.writeln('</div>');
      return buf.toString();
      ...
      final instruction = _stringFrom(content, const ['instruction', 'prompt', 'title']) ?? 'Ordena los elementos';
      final buf = StringBuffer('<div class="block sorting-block"><h3>$instruction</h3><ul id="sort-$safeId" class="sortable-list">');
      for (final item in items) {
        final map = (item is Map) ? item.cast<String, dynamic>() : <String, dynamic>{};
      ...
      final rawText = (_stringFrom(content, const ['text', 'statement', 'prompt']) ?? '').toString();
      ...
      if (leftItems.isEmpty || rightItems.isEmpty) {
        ...
        if (pairs.isNotEmpty) {
          ...
          for (var i = 0; i < pairs.length; i++) {
            final pair = (pairs[i] is Map) ? (pairs[i] as Map).cast<String, dynamic>() : <String, dynamic>{};
            leftItems.add({'id': 'l$i', 'text': pair['left'] ?? pair['a'] ?? pair['term'] ?? ''});
            rightItems.add({'id': 'r$i', 'text': pair['right'] ?? pair['b'] ?? pair['definition'] ?? ''});
          }
        }
      }
      final buf = StringBuffer('<div class="block matching-block" id="match-$safeId"><h3>Relacionar conceptos</h3><div class="matching-list">');
      var matchRowIndex = 0;
      for (final left in leftItems) {
        final leftMap = (left is Map) ? left.cast<String, dynamic>() : <String, dynamic>{};
        final leftId = leftMap['id'] ?? '';
        final leftText = esc.convert(leftMap['text']?.toString() ?? '');
        final selectId = 'input-$safeId-$matchRowIndex';
        buf.writeln('<div class="matching-row"><span class="match-label">$leftText</span><select id="$selectId" name="input-$safeId-$matchRowIndex" data-correct="$leftId"><option value="">Selecciona...</option>');
        for (final right in rightItems) {
          final rightMap = (right is Map) ? right.cast<String, dynamic>() : <String, dynamic>{};
      ...
      buf.writeln('</div><button class="btn-primary" onclick="checkMatching(\'$safeId\')">Comprobar</button><div id="fb-$safeId" class="feedback"></div></div>');
      return buf.toString();
      ...
      if (cards.isNotEmpty) {
        final firstCard = (cards.first is Map) ? (cards.first as Map).cast<String, dynamic>() : <String, dynamic>{};
        final front = esc.convert((firstCard['frontText'] ?? firstCard['question'] ?? firstCard['front'] ?? '').toString());
        final back = esc.convert((firstCard['backText'] ?? firstCard['answer'] ?? firstCard['back'] ?? '').toString());
      ...
      final front = _stringFrom(content, const ['front', 'frontText', 'question']) ?? '';
      final back = _stringFrom(content, const ['back', 'backText', 'answer']) ?? '';
      ...
      final question = esc.convert(_stringFrom(content, const ['question', 'prompt', 'title']) ?? 'Pregunta');
      final options = _listFrom(content, const ['options', 'choices', 'answers']);
      final correctIndices = List.from(content['correctIndices'] ?? (content['correctIndex'] != null ? [content['correctIndex']] : (content['correctAnswer'] != null ? [content['correctAnswer']] : [])));
      ...
      final buf = StringBuffer('<div class="block quiz-block"><h3>$question</h3><div class="options">');
      for (int i = 0; i < options.length; i++) {
        buf.writeln('<label><input type="$inputType" id="input-$safeId-$i" name="input-$safeId-$i" value="$i"> ${esc.convert(options[i].toString())}</label>');
      }
      buf.writeln('</div><button class="btn-primary" onclick=\'checkQuiz("$safeId", $jsArray)\'>Comprobar</button><div id="fb-$safeId" class="feedback"></div></div>');
      return buf.toString();
      ...
      final introText = esc.convert(_stringFrom(content, const ['introText', 'prompt', 'description', 'text']) ?? 'Situacion a resolver');
      final imagePath = _resolveAsset(_stringFrom(content, const ['imagePath', 'path', 'url', 'src']) ?? '', assetMap);
      final options = _listFrom(content, const ['options', 'choices', 'answers']);
      final buf = StringBuffer('<div class="block scenario-block" id="scenario-$safeId"><h3>Escenario</h3><p>$introText</p>');
      if (imagePath.isNotEmpty) {
        buf.writeln('<div class="media-block center"><img src="$imagePath" alt="Escenario"></div>');
      }
      buf.writeln('<div class="scenario-options">');
      for (final option in options) {
        final optMap = (option is Map) ? option.cast<String, dynamic>() : <String, dynamic>{};
        ...
        buf.writeln('<button class="btn-primary" onclick="selectScenario(\'$safeId\', ${isCorrect ? 'true' : 'false'}, $feedback)">$text</button>');
      }
      buf.writeln('</div><div id="scenario-fb-$safeId" class="feedback"></div></div>');
      return buf.toString();
      ...
      final buf = StringBuffer('<div class="block media-block center"><div class="carousel">');
      for (final item in items) {
        final map = (item is Map) ? item.cast<String, dynamic>() : <String, dynamic>{};
        final url = _resolveAsset((map['url'] ?? map['image'] ?? map['src'] ?? '').toString(), assetMap);
      ...
      final descA = (itemA['description'] ?? itemA['desc'] ?? itemA['content'] ?? '').toString();
      final descB = (itemB['description'] ?? itemB['desc'] ?? itemB['content'] ?? '').toString();
  ...
  Map<String, dynamic> _mapFrom(Map<String, dynamic> content, List<String> keys) {
    for (final key in keys) {
      final value = content[key];
  ...
  String _hotspotListHtml(List<dynamic> hotspots) {
    ...
    for (final spot in hotspots) {
      final map = (spot is Map) ? spot.cast<String, dynamic>() : <String, dynamic>{};
      final label = (map['label'] ?? map['title'] ?? map['text'] ?? '').toString();
  ...
  String? _extractTextValue(Map<String, dynamic> content) {
    final direct = _stringFrom(content, const ['text', 'content', 'html', 'description']);
  ...
  String _renderFillBlanksText(String rawText, {required String blockId}) {
    ...
    for (final match in regex.allMatches(rawText)) {
      ...
      buffer.write('<input type="text" id="input-$safeId-$blankIndex" name="input-$safeId-$blankIndex" class="blank-input" data-answer="$safeAnswer">');
      blankIndex++;
      lastIndex = end;
    }
  ...
  String generateManifest(
    ...
    if (assetFiles != null && assetFiles.isNotEmpty) {
      for (int i = 0; i < assetFiles.length; i++) {
        ...
        resourcesDefs.writeln('<resource identifier="RES-ASSET-$i" type="webcontent" adlcp:scormtype="asset" href="$safePath"><file href="$safePath"/></resource>');
      }
    }
